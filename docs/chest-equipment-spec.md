...
現在はカテゴリごとに「同カテゴリ内の定義順」で段階成長する。
`equipmentMaster.json` 上で後ろにあるアイテムほど強い。

| カテゴリ | 成長ルール |
|----------|------------|
| 片手剣 | `atk` が 1段階ごとに `+10` |
| 短剣 | `atk` が 1段階ごとに `+7`、`spd` が `+3` |
| 投刃 | `atk` が 1段階ごとに `+8`、`spd` が `+2` |
| 両手斧 | `atk` が 1段階ごとに `+12` |
| 両手槌 | `atk` が 1段階ごとに `+10`、`def` が `+4` |
| 弓 | `atk` が 1段階ごとに `+9`、`spd` が `+2` |
| 杖 | `spi` が 1段階ごとに `+10`、`mp` が `+6` |
| 盾 | `hp` が 1段階ごとに `+20`、`def` が `+8` |

例:

- 片手剣1番目 `bronze_sword` は `atk +10`
- 片手剣2番目 `iron_sword` は `atk +20`
- 片手剣31番目 `chest_chaos_sword` は `atk +310`
