...
describe("game/dungeonBundles", () => {
  it("builds boss-based bundles for crestoria 1-200", () => {
  ...
  it("finds bundle by floor", () => {
    expect(findBundleByFloor("crestoria_dungeon_1_200", 1)).toEqual({ startFloor: 1, bossFloor: 5 });
    expect(findBundleByFloor("crestoria_dungeon_1_200", 19)).toEqual({ startFloor: 11, bossFloor: 20 });
    expect(findBundleByFloor("crestoria_dungeon_1_200", 3)).toEqual({ startFloor: 1, bossFloor: 5 });
  });
});
